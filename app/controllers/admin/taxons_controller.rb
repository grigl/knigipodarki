class Admin::TaxonsController < Admin::BaseController
  ...
  def selected
    ...
    @all_taxons = Taxon.all.select { |taxon| taxon.parent_id && !taxon.products.include?(@product) }

    respond_with(:admin, @taxons, @all_taxons)
  end
  ...
  def select
    ...
    if @taxon.parent_id? && @taxon.parent.parent_id?
      new_taxon = Taxon.find(@taxon.parent_id)
      @product.taxons << new_taxon unless @product.taxons.include?(new_taxon)
    end
    @product.save
    @taxons = @product.taxons
